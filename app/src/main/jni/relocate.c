...
static int
relocateInstructionInThumb16(uint32_t pc, uint16_t instruction, uint16_t *trampoline_instructions) {
    int type;
    int offset;
...
static int
relocateInstructionInThumb32(uint32_t pc, uint16_t high_instruction, uint16_t low_instruction,
    ...
    if (type == BLX_THUMB32 || type == BL_THUMB32 || type == B1_THUMB32 || type == B2_THUMB32) {
        ...
        trampoline_instructions[idx++] = 0xF000;    // LDR.W PC, [PC]
        if (type == BLX_THUMB32) {
            x = (s << 24) | (i1 << 23) | (i2 << 22) | ((high_instruction & 0x3FF) << 12) |
                ((low_instruction & 0x7FE) << 1);
            imm32 = s ? (x | (0xFFFFFFFF << 25)) : x;
            value = pc + imm32;
        } else if (type == BL_THUMB32) {
            x = (s << 24) | (i1 << 23) | (i2 << 22) | ((high_instruction & 0x3FF) << 12) |
                ((low_instruction & 0x7FF) << 1);
            imm32 = s ? (x | (0xFFFFFFFF << 25)) : x;
            value = pc + imm32 + 1;
        } else if (type == B1_THUMB32) {
            x = (s << 20) | (j2 << 19) | (j1 << 18) | ((high_instruction & 0x3F) << 12) |
                ((low_instruction & 0x7FF) << 1);
...
relocateInstructionInThumb(uint32_t target_addr, uint16_t *orig_instructions, int length,
    ...
    while (1) {
        ...
        if ((orig_instructions[orig_pos] >> 11) >= 0x1D &&
            (orig_instructions[orig_pos] >> 11) <= 0x1F) {
            if (orig_pos + 2 > length / sizeof(uint16_t)) {
                break;
            }
            offset = relocateInstructionInThumb32(pc, orig_instructions[orig_pos],
            ...
            offset = relocateInstructionInThumb16(pc, orig_instructions[orig_pos],
                                                  &trampoline_instructions[trampoline_pos]);
            pc += sizeof(uint16_t);
            trampoline_pos += offset;
            ++orig_pos;
...
void relocateInstruction(uint32_t target_addr, void *orig_instructions, int length,
                         ...
                         int *trampoline_boundaries, int *count) {
    LOGD("relocateInstruction::%ld, %p, %p, %p, %p", length, trampoline_instructions,
         orig_boundaries, trampoline_boundaries, count);

    LOGD("relocateInstruction thumb:%d", (target_addr & 1));
    if (target_addr & 1 == 1) {
